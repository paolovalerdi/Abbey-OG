...
import com.paolovalerdi.abbey.model.MediaReader;
...
public class SongRepository {
    ...
    protected static final String[] BASE_PROJECTION = new String[]{
            ...
            AUDIO_COLUMNS_ALBUM_ARTIST//12
    };

    private static final int BATCH_SIZE = 900; // used in createSongCursor* functions. SQLite limit on the number of ?argument is 999, we leave some to the other call sites.
    ...
    private static Song getSongFromCursorImpl(@NonNull Cursor cursor) {
        ...
        String artistName = cursor.getString(11);
        String albumArtist = cursor.getString(12);

        if (albumArtist == null) albumArtist = "Unknown";
        if (artistName == null) artistName= "Unknown";

        return new Song(id, title, trackNumber, year, duration, data, dateAdded, dateModified, albumId, albumName, artistId, artistName, albumArtist);
    }
    ...
    public static Cursor makeSongCursor(@NonNull final Context context, @Nullable final String selection, final String[] selectionValues) {
        return makeSongCursor(context, selection, selectionValues, PreferenceUtil.INSTANCE.getSongSortOrder());
    }
